...
public class InsuredPersonRepositoryTests {
    ...
    @Test
    public void InsuredPersonRepository_FindByEmail_ReturnInsuredPerson() {
        InsuredPerson foundedInsuredPerson = insuredPersonRepository.findByEmail("nikam.se.nehnu@proc_rucnik.com").get();

        Assertions.assertThat(foundedInsuredPerson).isNotNull();
        Assertions.assertThat(foundedInsuredPerson).isEqualTo(insuredPerson_1);
    }
    ...
    public void InsuredPersonRepository_FindAllByCreatedById_ReturnInsuredPersonsPage() {
        ...
        List<InsuredPerson> insuredPersonsList = foundPage.getContent();

        Assertions.assertThat(foundPage).isNotNull();
        Assertions.assertThat(foundPage.getContent().size()).isGreaterThan(0);
        Assertions.assertThat(foundPage.getContent().get(0)).isInstanceOf(InsuredPerson.class);

        for (InsuredPerson ip : insuredPersonsList) {
            Assertions.assertThat(ip.getCreatedBy()).isEqualTo(foundUser);
        }
